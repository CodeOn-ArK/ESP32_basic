...
static BLEUUID serviceUUID("Enter Service UUID here");
// The characteristic of the remote service we are interested in.
static BLEUUID charUUID("Enter Characteristic UUID here");

static boolean doConnect = false;
static boolean connected = false;
static boole an doScan = false;
static BLERemoteCharacteristic* pRemoteCharacteristic;
static BLEAdvertisedDevice* myDevice;

// callback for notification defined here
static void notifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify) {

  Serial.print("Notify callback for characteristic ");
  Serial.print(pBLERemoteCharacteristic->getUUID().toString().c_str());     //Print the service UUID from the SERVER
...
class MyClientCallback : public BLEClientCallbacks {                        //Callback for the client side
  void onConnect(BLEClient* pclient) {}

  void onDisconnect(BLEClient* pclient) {                                   //Called when disconnected
    connected = false;
